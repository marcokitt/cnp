#!/usr/bin/env python3
"""
Cyber Lab Orchestrator v6.4.4

Focus:
- Auto-detects CALDERA API mode (v2 preferred, legacy /api/rest fallback)
- Polls operation links/potential-links/report/event-logs when available
- Decision counter uses links + potential-links when present to avoid stale zero values
- Seed-only Windows adversary mode to encourage batch-planner link creation
- Filters out abilities with unsatisfied requirements and unresolved placeholders
- Prefers noisy Windows abilities likely to generate Sysmon/Wazuh telemetry
- Robust handling of UUID/string operation IDs
- Live monitor with:
  * time progress bar
  * decision/link progress bar
  * completed/success/error counters
...
class DebugLogger:
    ...
    def log(self, event: str, **fields: Any) -> None:
        if not self.enabled or self.path is None:
            return
        payload = {"ts": now_utc(), "event": event, **fields}
...
class LabConfig:
    ...
    def from_file(path: str) -> "LabConfig":
        ...
        return LabConfig(
            ...
            caldera_api_mode=str(data["caldera"].get("api_mode", "auto")).lower(),
            ...
            launch_monitor_window=bool(data.get("runtime", {}).get("launch_monitor_window", False)),
            ...
            seed_only_mode=bool(data.get("runtime", {}).get("seed_only_mode", True)),
            debug_caldera=bool(data.get("runtime", {}).get("debug_caldera", True)),
            debug_wazuh=bool(data.get("runtime", {}).get("debug_wazuh", True)),
            keep_wazuh_raw_lines=bool(data.get("runtime", {}).get("keep_wazuh_raw_lines", True)),
        )
...
class CalderaClient:
    def __init__(self, cfg: LabConfig) -> None:
        ...
        self.debug = DebugLogger(None, False)
        self.api_mode = self._detect_api_mode()

    def set_debug_dir(self, session_dir: Path) -> None:
        self.debug = DebugLogger(session_dir / "debug_caldera.ndjson", self.cfg.debug_caldera)

    def _debug_preview(self, data: Any, limit: int = 400) -> str:
        try:
    ...
    def _detect_api_mode(self) -> str:
        preferred = (self.cfg.caldera_api_mode or "auto").lower()
        if preferred in {"legacy", "rest"}:
            return "legacy"
        if preferred == "v2":
            return "v2"
        try:
            r = self.session.get(f"{self.base}/api/v2/operations", timeout=10)
    ...
    def _json_or_none(self, response: requests.Response) -> Any:
        ...
        if "application/json" in ctype or response.text.strip().startswith(("{", "[")):
            return response.json()
        return {"raw_text": response.text}

    def rest(self, method: str, payload: Dict[str, Any]) -> Any:
        url = f"{self.base}/api/rest"
        self.debug.log("caldera_request", api="legacy", method=method, endpoint="/api/rest", payload_preview=self._debug_preview(payload))
        try:
            response = self.session.request(method=method, url=url, data=json.dumps(payload), timeout=30)
            body = self._json_or_none(response) or {"status": "ok"}
            self.debug.log("caldera_response", api="legacy", method=method, endpoint="/api/rest", status=response.status_code, body_preview=self._debug_preview(body))
            response.raise_for_status()
            return body
        except Exception as exc:
            self.debug.log("caldera_error", api="legacy", method=method, endpoint="/api/rest", error=str(exc))
            raise

    def v2(self, method: str, endpoint: str, payload: Optional[Dict[str, Any]] = None, timeout: int = 30) -> Any:
        ep = f"/api/v2/{endpoint.lstrip('/')}"
        url = f"{self.base}{ep}"
        self.debug.log("caldera_request", api="v2", method=method, endpoint=ep, payload_preview=self._debug_preview(payload))
        try:
            response = self.session.request(method=method, url=url, json=payload, timeout=timeout)
            body = self._json_or_none(response) or {"status": "ok"}
            self.debug.log("caldera_response", api="v2", method=method, endpoint=ep, status=response.status_code, body_preview=self._debug_preview(body))
            response.raise_for_status()
            return body
        except Exception as exc:
            self.debug.log("caldera_error", api="v2", method=method, endpoint=ep, error=str(exc))
            raise
    ...
    def list_adversaries(self) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", "adversaries"))
            except Exception:
                pass
    ...
    def list_agents(self) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", "agents"))
            except Exception:
                pass
    ...
    def list_operations(self) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", "operations"))
            except Exception:
                pass
    ...
    def list_abilities(self) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", "abilities"))
            except Exception:
                pass
    ...
    def get_operation(self, op_id: Any) -> Optional[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                data = self.v2("GET", f"operations/{op_id}")
                if isinstance(data, dict):
                    return data
            except Exception:
                pass
    ...
    def set_operation_state(self, op_id: Any, state: str, autonomous: Optional[bool] = None) -> Any:
        if self.api_mode == "v2":
            payload: Dict[str, Any] = {"state": state}
            if autonomous is not None:
                payload["autonomous"] = 1 if autonomous else 0
            try:
                return self.v2("PATCH", f"operations/{op_id}", payload)
            except Exception:
                pass
        return self.rest("POST", {"index": "operation", "op_id": op_id, "state": state})

    def create_operation(self, name: str, adversary_id: str, group: str) -> Dict[str, Any]:
        self.debug.log("create_operation_attempt", api_mode=self.api_mode, name=name, adversary_id=adversary_id, group=group)
        if self.api_mode == "v2":
            payload = {
                "name": name,
                "autonomous": 1,
            ...
            try:
                data = self.v2("POST", "operations", payload)
                return data if isinstance(data, dict) else {"raw": data}
            except Exception as exc:
                self.debug.log("create_operation_v2_fallback", error=str(exc))
        return self.create_operation_legacy(name, adversary_id, group)
    ...
    def save_adversary(self, adversary: Dict[str, Any]) -> Any:
        if self.api_mode == "v2":
            try:
                return self.v2("POST", "adversaries", adversary)
            except Exception:
                pass
        return self.rest("PUT", {"index": "adversaries", "adversary": adversary})

    def get_operation_links(self, op_id: Any) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", f"operations/{op_id}/links"))
            except Exception:
                return []
        op = self.get_operation(op_id) or {}
        host_group = op.get("host_group") or []
        links: List[Dict[str, Any]] = []
        if isinstance(host_group, list):
            for agent in host_group:
                if isinstance(agent, dict):
                    links.extend(agent.get("links", []) or [])
        return links

    def get_operation_potential_links(self, op_id: Any) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                return self._as_list(self.v2("GET", f"operations/{op_id}/potential-links"))
            except Exception:
                return []
        return []

    def get_operation_report(self, op_id: Any) -> Dict[str, Any]:
        if self.api_mode == "v2":
            try:
                data = self.v2("GET", f"operations/{op_id}/report")
                return data if isinstance(data, dict) else {"raw": data}
            except Exception:
                return {}
        return {}

    def get_operation_event_logs(self, op_id: Any) -> List[Dict[str, Any]]:
        if self.api_mode == "v2":
            try:
                data = self.v2("GET", f"operations/{op_id}/event-logs")
    ...
    def choose_agent(self, interactive: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if self.cfg.target_agent_paw:
            for agent in agents:
                if str(agent.get("paw")) == str(self.cfg.target_agent_paw):
                    return agent
        if not interactive:
            return agents[0]
        print(f"\nAvailable agents from Caldera (api_mode={self.api_mode}):")
...
def unresolved_placeholders(ability: Dict[str, Any]) -> List[str]:
    cmd = get_windows_executor_command(ability)
    return PLACEHOLDER_RE.findall(cmd)


def has_requirements(ability: Dict[str, Any]) -> bool:
...
def noisy_score(ability: Dict[str, Any]) -> int:
    ...
    if technique in {"T1033", "T1057", "T1018", "T1518.001", "T1069.001", "T1135", "T1087.001", "T1016"}:
        score += 2
    if "clear-history" in cmd or cmd.strip().lower() == "clear-history;clear":
        score -= 4
    return score


def seed_ready(ability: Dict[str, Any]) -> Tuple[bool, str]:
    if has_requirements(ability):
        return False, "has requirements"
    unresolved = unresolved_placeholders(ability)
    if unresolved:
        return False, f"unresolved placeholders: {', '.join(unresolved[:3])}"
    cmd = get_windows_executor_command(ability).strip()
    if not cmd:
        return False, "empty windows command"
    return True, "seed-ready"


def compatibility_report(adversary: Dict[str, Any], agent: Dict[str, Any], abilities: List[Dict[str, Any]], noisy_only: bool = False, seed_only: bool = False) -> Dict[str, Any]:
    ...
    for ref in refs:
        ...
        item = {
            ...
            "windows_command": get_windows_executor_command(ability),
            "has_requirements": has_requirements(ability),
            "unresolved_placeholders": unresolved_placeholders(ability),
        }
        if ok:
            seed_ok, seed_reason = seed_ready(ability)
            item["seed_ready"] = seed_ok
            item["seed_reason"] = seed_reason
            if seed_only and not seed_ok:
                item["reason"] = f"compatible but filtered by seed_only_mode ({seed_reason})"
                incompatible.append(item)
            elif noisy_only and item["noisy_score"] <= 0:
                item["reason"] = "compatible but filtered by noisy_only_mode"
                incompatible.append(item)
            else:
                compatible.append(item)
        else:
            item["seed_ready"] = False
            item["seed_reason"] = "not compatible"
            incompatible.append(item)

    compatible.sort(key=lambda x: (-x["noisy_score"], x["has_requirements"], str(x["name"])))
    return {
        ...
        "seed_only_mode": seed_only,
    }


def build_temp_adversary_from_compatible(agent: Dict[str, Any], source_adversary: Dict[str, Any], abilities: List[Dict[str, Any]], max_items: int, noisy_only: bool, seed_only: bool) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ability_map = {str(a.get("ability_id") or a.get("id")): a for a in abilities}
    ...
    for score, ability in candidates[:max_items]:
        ...
        selected_refs.append({
            ...
            "technique_id": ability.get("technique_id"),
            "noisy_score": score,
            "command_preview": truncate(get_windows_executor_command(ability), 100),
        })

    temp_adv_id = f"temp-seed-{sanitize_name(str(agent.get('paw')))}-{int(time.time())}"
    temp_adv = {
        "adversary_id": temp_adv_id,
        "name": f"Temp Seed Windows - {source_adversary.get('name') or source_adversary.get('adversary_id')}",
        "description": f"Auto-generated seed adversary from {source_adversary.get('adversary_id') or source_adversary.get('id')} for agent {agent.get('paw')}",
        "atomic_ordering": selected_ids,
    }
    return temp_adv, selected_refs


def rank_adversaries_for_agent(adversaries: List[Dict[str, Any]], agent: Dict[str, Any], abilities: List[Dict[str, Any]], noisy_only: bool, seed_only: bool) -> List[Dict[str, Any]]:
    ranked = []
    for adv in adversaries:
        compat = compatibility_report(adv, agent, abilities, noisy_only=noisy_only, seed_only=seed_only)
        ranked.append({"adversary": adv, "compatibility": compat})
    ranked.sort(key=lambda x: (-x["compatibility"]["compatible_count"], x["compatibility"]["missing_count"], str(x["adversary"].get("name") or x["adversary"].get("adversary_id") or "")))
    return ranked
...
def choose_ranked_adversary(ranked: List[Dict[str, Any]], interactive: bool = False) -> Optional[Dict[str, Any]]:
    ...
    for i, item in enumerate(ranked, start=1):
        ...
        print(f"{i:>2}. {truncate(adv_id,24):<24} | {truncate(adv.get('name',adv_id),28):<28} | compatible: {compat['compatible_count']:<3} | missing: {compat['missing_count']:<3} | seed_only={compat['seed_only_mode']}")
    while True:
...
class WazuhTailMonitor(threading.Thread):
    def __init__(self, cfg: LabConfig, line_queue: "queue.Queue[Tuple[str, Dict[str, Any]]]", stop_event: threading.Event, debug_logger: Optional[DebugLogger] = None):
        ...
        self.debug = debug_logger or DebugLogger(None, False)
        self.raw_samples: "deque[str]" = deque(maxlen=50)
    ...
    def connect(self) -> None:
        ...
        self.debug.log("wazuh_connect_attempt", ssh=safe_kwargs, command=self.command)
        self.client.connect(**kwargs)
        self.debug.log("wazuh_connect_ok", host=self.cfg.wazuh_host, port=self.cfg.wazuh_port)
    ...
    def run(self) -> None:
        try:
            ...
            self.debug.log("wazuh_tail_started", command=self.command)
            buffer = ""
            while not self.stop_event.is_set():
                if channel.recv_ready():
                    ...
                    while "\n" in buffer:
                        ...
                        if not line:
                            continue
                        if self.cfg.keep_wazuh_raw_lines:
                            self.raw_samples.append(line[:1000])
                        parsed = self.try_parse(line)
                        if parsed.get("_parse_error"):
                            self.debug.log("wazuh_parse_error", preview=line[:400])
                        else:
                            self.debug.log("wazuh_line", preview=line[:400])
                        self.line_queue.put((line, parsed))
                else:
                    time.sleep(0.2)
        except Exception as exc:
            self.debug.log("wazuh_monitor_error", error=str(exc))
    ...
    def try_parse(line: str) -> Dict[str, Any]:
        try:
            ...
            return {"raw": line, "_parse_error": True}



class OperationPoller(threading.Thread):
    def __init__(self, client: CalderaClient, op_id: Any, refresh_seconds: int, data_holder: Dict[str, Any], stop_event: threading.Event, debug_logger: Optional[DebugLogger] = None):
        super().__init__(daemon=True)
        self.client = client
        self.op_id = op_id
    ...
    def run(self) -> None:
        self.data_holder["api_mode"] = getattr(self.client, "api_mode", "unknown")
        self.data_holder.setdefault("poll_history", [])
        while not self.stop_event.is_set():
            try:
                ...
                try:
                    links = self.client.get_operation_links(self.op_id)
                    if isinstance(links, list):
                        self.data_holder["links"] = links
                except Exception as exc:
                    self.data_holder["links_error"] = str(exc)
                try:
                    potential = self.client.get_operation_potential_links(self.op_id)
                    if isinstance(potential, list):
                        self.data_holder["potential_links"] = potential
                except Exception as exc:
                    self.data_holder["potential_links_error"] = str(exc)
                try:
                    report = self.client.get_operation_report(self.op_id)
                    if isinstance(report, dict) and report:
                        self.data_holder["report"] = report
                except Exception as exc:
                    self.data_holder["report_error"] = str(exc)
                try:
                    event_logs = self.client.get_operation_event_logs(self.op_id)
                    if isinstance(event_logs, list):
                        self.data_holder["event_logs"] = event_logs
                except Exception as exc:
                    self.data_holder["event_logs_error"] = str(exc)

                snapshot = {
                    "ts": now_utc(),
                    "state": ((self.data_holder.get("operation") or {}).get("state") or (self.data_holder.get("operation") or {}).get("status")),
                    "links": len(self.data_holder.get("links") or []),
                    "potential_links": len(self.data_holder.get("potential_links") or []),
                    "event_logs": len(self.data_holder.get("event_logs") or []),
                }
                self.data_holder["poll_history"] = (self.data_holder.get("poll_history") or [])[-199:] + [snapshot]
                self.debug.log("operation_poll_snapshot", **snapshot)
            except Exception as exc:
                self.data_holder["poll_error"] = str(exc)
                self.debug.log("operation_poll_error", error=str(exc))
            time.sleep(self.refresh_seconds)



def summarize_link_statuses(links: List[Dict[str, Any]]) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for link in links or []:
        if not isinstance(link, dict):
            continue
        key = str(link.get("status", "unknown"))
        counts[key] = counts.get(key, 0) + 1
    return counts


def build_operation_diagnostics(op_status: Dict[str, Any], selector_info: Optional[Dict[str, Any]], compat: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    op = (op_status or {}).get("operation") or {}
    links = [x for x in (op_status or {}).get("links", []) if isinstance(x, dict)]
    potential = [x for x in (op_status or {}).get("potential_links", []) if isinstance(x, dict)]
    event_logs = [x for x in (op_status or {}).get("event_logs", []) if isinstance(x, dict)]
    selected_agent = ((selector_info or {}).get("selected_agent") or {})
    selected_paw = selected_agent.get("paw")
    selected_group = selected_agent.get("group")
    filtered_links = []
    for link in links:
        paw = link.get("paw") or deep_get(link, ["agent", "paw"])
        if selected_paw and paw and paw != selected_paw:
            continue
        filtered_links.append(link)

    status_counts = summarize_link_statuses(filtered_links)
    state = op.get("state") or op.get("status") or "unknown"
    host_group = op.get("host_group")
    host_group_count = len(host_group) if isinstance(host_group, list) else 0
    diag = {
        "api_mode": op_status.get("api_mode"),
        "operation_id": op.get("id") or op_status.get("op_id"),
        "operation_name": op.get("name"),
        "operation_state": state,
        "selected_agent_paw": selected_paw,
        "selected_group": selected_group,
        "host_group_count": host_group_count,
        "compatible_count": (compat or {}).get("compatible_count"),
        "referenced_ability_count": (compat or {}).get("referenced_ability_count"),
        "links_total": len(links),
        "links_relevant_to_selected_agent": len(filtered_links),
        "potential_links_total": len(potential),
        "event_logs_total": len(event_logs),
        "report_present": isinstance(op_status.get("report"), dict) and bool(op_status.get("report")),
        "link_status_counts": status_counts,
        "errors": {k: v for k, v in (op_status or {}).items() if k.endswith("_error") or k in {"poll_error", "monitor_error"}},
        "probable_causes": [],
    }

    if (compat or {}).get("compatible_count") == 0:
        diag["probable_causes"].append("Zero ability compatibili dopo filtri noisy/seed.")
    if host_group_count == 0:
        diag["probable_causes"].append("Operation senza host_group: l'agente potrebbe non essere entrato nell'operation.")
    if state not in {"running", "run_one_link"}:
        diag["probable_causes"].append(f"Operation in stato {state}, non propriamente in esecuzione.")
    if len(filtered_links) == 0 and len(potential) == 0:
        diag["probable_causes"].append("Nessun link né potential-link: planner fermo, avversario vuoto o requisiti non soddisfatti.")
    if len(filtered_links) == 0 and len(potential) > 0:
        diag["probable_causes"].append("Esistono potential-links ma non links: planner attivo, ma esecuzione non partita o blocchi lato agente.")
    if len(filtered_links) > 0 and sum(status_counts.values()) > 0 and status_counts.get("0", 0) == 0 and status_counts.get("success", 0) == 0:
        diag["probable_causes"].append("Links creati ma senza successi: verificare executor, privilegi e payload lato agente.")
    if op_status.get("event_logs_error") and not event_logs:
        diag["probable_causes"].append("Endpoint event-logs non disponibile o non popolato.")
    return diag


def load_session_context(session_dir: Path) -> Dict[str, Any]:
    context: Dict[str, Any] = {}
    for name in ("operation_info.json", "compatibility_report.json", "selector_info.json", "operation_info_final.json", "caldera_links.json", "caldera_potential_links.json", "caldera_report.json", "caldera_event_logs.json"):
        p = session_dir / name
        if p.exists():
            try:
...
def derive_target_markers(cfg: LabConfig, session_dir: Optional[Path] = None) -> Set[str]:
    ...
    if session_dir is not None:
        ...
        for value in [selected_agent.get("host"), selected_agent.get("paw"), selected_agent.get("username"), selected_agent.get("display_name"), selected_agent.get("location")]:
            if value:
        ...
        if isinstance(ordering, list):
            for item in ordering:
                if isinstance(item, str):
                    ...
                    ids.add(str(item.get("ability_id") or item.get("id")))
        temp_info = selector_info.get("temp_adversary_info") or {}
        temp_adv = temp_info.get("adversary") or {}
        ordering = temp_adv.get("atomic_ordering") or []
        if isinstance(ordering, list):
            for item in ordering:
                if isinstance(item, str):
                    ids.add(item)
...
def decision_metrics(op: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None, op_status: Optional[Dict[str, Any]] = None) -> Dict[str, int]:
    ...
    metrics = {"planned_total": len(relevant_ids), "created": 0, "finished": 0, "success": 0, "error": 0, "potential": 0}

    links_from_status = []
    potential_from_status = []
    if op_status:
        if isinstance(op_status.get("links"), list):
            links_from_status = op_status.get("links") or []
        if isinstance(op_status.get("potential_links"), list):
            potential_from_status = op_status.get("potential_links") or []

    if links_from_status:
        seen_links: Set[str] = set()
        for link in links_from_status:
            if not isinstance(link, dict):
                continue
            paw = link.get("paw") or deep_get(link, ["agent", "paw"])
            if selected_paw and paw and paw != selected_paw:
            ...
            ability_id = str(link.get("ability_id") or ability.get("ability_id") or ability.get("id") or "")
            if relevant_ids and ability_id and ability_id not in relevant_ids:
                continue
            metrics["created"] += 1
            if link.get("finish") or link.get("finished"):
                metrics["finished"] += 1
                status = link.get("status")
                if status in [0, "0", None, "success", "completed"]:
                    metrics["success"] += 1
                else:
                    metrics["error"] += 1
        metrics["potential"] = sum(1 for x in potential_from_status if isinstance(x, dict))
        if metrics["created"] == 0 and metrics["potential"] > 0:
            metrics["created"] = metrics["potential"]
        if metrics["planned_total"] == 0:
            metrics["planned_total"] = max(metrics["created"], metrics["finished"], metrics["potential"])
        return metrics
...
def infer_source_ip(alert: Dict[str, Any], cfg: LabConfig) -> str:
    preferred_paths = [["data", "srcip"], ["srcip"], ["win", "eventdata", "sourceIp"], ["win", "eventdata", "SourceIp"], ["sysmon", "event_data", "SourceIp"], ["network", "srcip"]]
    candidates: List[str] = []
...
def render_live_screen(session_name: str, remaining: int, total: int, observed_count: int, recent_rows: List[Dict[str, str]], op_status: Dict[str, Any], compat: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> None:
    ...
    dmetrics = decision_metrics(op, selector_info, op_status)
    print("CALDERA")
    print("-" * 120)
    print(f"op_id={op_id} | state={state} | group={host_group} | chain={len(chain) if isinstance(chain, list) else 'n/a'} | facts={len(facts) if isinstance(facts, list) else 'n/a'} | api_mode={op_status.get('api_mode','unknown')}")
    print(f"decisions: {decision_progress_bar(dmetrics['created'], dmetrics['planned_total'])} | potential={dmetrics.get('potential',0)} | completed={dmetrics['finished']} | success={dmetrics['success']} | error={dmetrics['error']}")
    ...
    print("COMPATIBILITY")
    print("-" * 120)
    print(f"adversary={compat.get('adversary_id')} | agent={compat.get('agent_paw')} | platform={compat.get('agent_platform')} | compatible={compat.get('compatible_count', 0)} | incompatible={compat.get('incompatible_count', 0)} | missing={compat.get('missing_count', 0)} | noisy_only={compat.get('noisy_only_mode', False)} | seed_only={compat.get('seed_only_mode', False)}")
...
def extract_attack_techniques_from_link(link: Dict[str, Any]) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    if not isinstance(link, dict):
        return results
    ability = link.get("ability") or {}
    technique_id = str(link.get("technique_id") or ability.get("technique_id") or "").upper().strip()
    if not technique_id:
        return results
    results.append({
        "technique_id": technique_id,
        "technique_name": str(link.get("technique_name") or ability.get("technique_name") or ability.get("name") or "unknown"),
        "tactic": str(link.get("tactic") or ability.get("tactic") or "unknown"),
        "ability_id": str(link.get("ability_id") or ability.get("ability_id") or ability.get("id") or ""),
        "ability_name": ability_name(ability) if isinstance(ability, dict) else str(link.get("ability_name") or "unknown"),
        "status": link.get("status"),
        "paw": str(link.get("paw") or deep_get(link, ["agent", "paw"]) or ""),
        "executor": str(link.get("executor") or ""),
        "command": get_windows_executor_command(ability) if isinstance(ability, dict) else "",
        "origin": "executed_link",
    })
    return results


def extract_attack_techniques_from_compat(compat: Dict[str, Any]) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for item in compat.get("compatible", []) or []:
        if not isinstance(item, dict):
            continue
        technique_id = str(item.get("technique_id") or "").upper().strip()
        if not technique_id:
            continue
        results.append({
            "technique_id": technique_id,
            "technique_name": str(item.get("name") or item.get("technique_name") or "unknown"),
            "tactic": str(item.get("tactic") or "unknown"),
            "ability_id": str(item.get("ability_id") or ""),
            "ability_name": str(item.get("name") or "unknown"),
            "status": "planned",
            "paw": str(compat.get("agent_paw") or ""),
            "executor": ",".join(compat.get("agent_executors", []) or []),
            "command": str(item.get("windows_command") or ""),
            "origin": "planned_compatible",
        })
    return results


def merge_attack_techniques(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    merged: Dict[Tuple[str, str, str], Dict[str, Any]] = {}
    for item in items:
        if not isinstance(item, dict):
            continue
        tid = str(item.get("technique_id") or "").upper().strip()
        if not tid:
            continue
        tactic = str(item.get("tactic") or "unknown")
        ability_id = str(item.get("ability_id") or "")
        key = (tid, tactic, ability_id)
        entry = merged.setdefault(key, {
            "technique_id": tid,
            "technique_name": str(item.get("technique_name") or item.get("ability_name") or "unknown"),
            "tactic": tactic,
            "ability_id": ability_id,
            "ability_name": str(item.get("ability_name") or item.get("technique_name") or "unknown"),
            "origins": set(),
            "statuses": set(),
            "paws": set(),
            "executors": set(),
        ...
        if origin:
            entry["origins"].add(str(origin))
        status = item.get("status")
        if status is not None:
            entry["statuses"].add(str(status))
        paw = item.get("paw")
        if paw:
            entry["paws"].add(str(paw))
        executor = item.get("executor")
        if executor:
            entry["executors"].add(str(executor))
        command = item.get("command")
        if command:
            entry["commands"].add(str(command))
    ...
    for _, entry in merged.items():
        rows.append({
            ...
            "paws": sorted(entry["paws"]),
            "executors": sorted(entry["executors"]),
            "commands": sorted(entry["commands"]),
        })
    rows.sort(key=lambda x: (x["tactic"], x["technique_id"], x["ability_name"]))
    return rows


def extract_operation_attack_summary(operation_info: Dict[str, Any], compat: Dict[str, Any], op_status: Optional[Dict[str, Any]], mapper: "D3fendMapper") -> Dict[str, Any]:
    raw_items: List[Dict[str, Any]] = []
    raw_items.extend(extract_attack_techniques_from_compat(compat))
    if op_status:
        for link in op_status.get("links", []) or []:
            raw_items.extend(extract_attack_techniques_from_link(link))
        for link in op_status.get("potential_links", []) or []:
            if not isinstance(link, dict):
                continue
            for item in extract_attack_techniques_from_link(link):
                item["origin"] = "potential_link"
                raw_items.append(item)
    merged = merge_attack_techniques(raw_items)
    attack_ids = {x["technique_id"] for x in merged}
    d3f_map = mapper.map_attack_ids(attack_ids) if attack_ids else {}
    for row in merged:
        row["d3fend"] = d3f_map.get(row["technique_id"], [])
    return {
        "operation_id": operation_info.get("id"),
        "operation_name": operation_info.get("name"),
        "technique_count": len(merged),
        "techniques": merged,
...
def generate_attack_html_report(output_path: Path, cfg: LabConfig, session_id: str, operation_info: Dict[str, Any], summary: Dict[str, Any]) -> None:
    rows: List[str] = []
    for item in summary.get("techniques", []) or []:
        technique_id = str(item.get("technique_id") or "")
        attack_url = f"{cfg.attack_lookup_base}{technique_id.replace('.', '/')}/"
        rows.append(
            "<tr>"
            f"<td><a href='{html.escape(attack_url)}'>{html.escape(technique_id)}</a></td>"
            f"<td>{html.escape(str(item.get('technique_name') or 'unknown'))}</td>"
            f"<td>{html.escape(str(item.get('tactic') or 'unknown'))}</td>"
            f"<td>{html.escape(str(item.get('ability_id') or ''))}</td>"
            f"<td>{html.escape(str(item.get('ability_name') or ''))}</td>"
            f"<td>{html.escape(', '.join(item.get('origins', []) or []))}</td>"
            f"<td>{html.escape(', '.join(item.get('statuses', []) or []))}</td>"
            f"<td>{html.escape(', '.join(item.get('paws', []) or []))}</td>"
            f"<td><code>{html.escape(truncate(' | '.join(item.get('commands', []) or []), 220))}</code></td>"
            "</tr>"
        )

    doc = f"""<!doctype html>
<html><head><meta charset=\"utf-8\"><title>MITRE ATT&CK Attack Report</title>
<style>
body {{ font-family: Arial, sans-serif; margin: 24px; background: #111; color: #eee; }}
a {{ color: #7db8ff; }}
table {{ border-collapse: collapse; width: 100%; background: #1c1c1c; }}
...
.badge {{ display: inline-block; padding: 4px 8px; border-radius: 999px; background: #222; border: 1px solid #444; margin-right: 6px; }}
</style></head><body>
<h1>MITRE ATT&CK Attack Report</h1>
<p><strong>Generated:</strong> {html.escape(now_utc())}</p>
<p><span class=\"badge\">Session {html.escape(session_id)}</span><span class=\"badge\">Operation {html.escape(str(operation_info.get('name') or 'unknown'))}</span><span class=\"badge\">Techniques {len(summary.get('techniques', []) or [])}</span></p>
<p>Questo report mette in fila le tecniche ATT&amp;CK pianificate, potenziali o realmente emerse dai link CALDERA. È la mappa dell'assalto, nuda e precisa.</p>
<table>
<tr><th>Technique ID</th><th>Name</th><th>Tactic</th><th>Ability ID</th><th>Ability Name</th><th>Origin</th><th>Status</th><th>PAW</th><th>Command preview</th></tr>
{''.join(rows) if rows else '<tr><td colspan="9">No ATT&amp;CK techniques extracted from CALDERA compatibility or operation links.</td></tr>'}
</table>
</body></html>"""
    output_path.write_text(doc, encoding="utf-8")


def generate_d3fend_html_report(output_path: Path, session_id: str, operation_info: Dict[str, Any], summary: Dict[str, Any], observed_attack_ids: Optional[Set[str]] = None) -> None:
    observed_attack_ids = observed_attack_ids or set()
    rows: List[str] = []
    for item in summary.get("techniques", []) or []:
        d3f = item.get("d3fend") or []
        coverage = "observed_in_alerts" if item.get("technique_id") in observed_attack_ids else "caldera_only"
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(item.get('technique_id') or ''))}</td>"
            f"<td>{html.escape(str(item.get('technique_name') or 'unknown'))}</td>"
            f"<td>{html.escape(str(item.get('tactic') or 'unknown'))}</td>"
            f"<td>{html.escape(', '.join(d3f) if d3f else 'No mapped D3FEND technique found')}</td>"
            f"<td>{html.escape(coverage)}</td>"
            "</tr>"
        )

    doc = f"""<!doctype html>
<html><head><meta charset=\"utf-8\"><title>D3FEND Defensive Mapping Report</title>
<style>
body {{ font-family: Arial, sans-serif; margin: 24px; background: #0f1115; color: #e8ecf1; }}
a {{ color: #7db8ff; }}
table {{ border-collapse: collapse; width: 100%; background: #171b22; }}
th, td {{ border: 1px solid #2a3340; padding: 10px; text-align: left; vertical-align: top; }}
th {{ background: #1e2630; }}
.panel {{ background: #171b22; border: 1px solid #2a3340; border-left: 6px solid #3f8cff; padding: 14px; margin: 14px 0; border-radius: 8px; }}
</style></head><body>
<h1>D3FEND Defensive Mapping Report</h1>
<p><strong>Generated:</strong> {html.escape(now_utc())}</p>
<div class=\"panel\"><strong>Session:</strong> {html.escape(session_id)}<br><strong>Operation:</strong> {html.escape(str(operation_info.get('name') or 'unknown'))}<br><strong>Observed ATT&amp;CK IDs in Wazuh:</strong> {html.escape(', '.join(sorted(observed_attack_ids)) if observed_attack_ids else 'none')}</div>
<p>Qui l'attacco incontra lo scudo: per ogni tecnica ATT&amp;CK emersa da CALDERA, il report prova a proiettarla sul vocabolario D3FEND. Non è magia: è correlazione utile, pronta per il debrief.</p>
<table>
<tr><th>Technique ID</th><th>Technique Name</th><th>Tactic</th><th>D3FEND mappings</th><th>Coverage</th></tr>
{''.join(rows) if rows else '<tr><td colspan="5">No D3FEND mappings could be produced because no ATT&amp;CK techniques were extracted.</td></tr>'}
</table>
</body></html>"""
    output_path.write_text(doc, encoding="utf-8")


def build_reports(cfg: LabConfig, mapper: D3fendMapper, session_dir: Path, session_id: str, operation_info: Dict[str, Any], observed_alerts: List[Dict[str, Any]], grouped: Dict[str, List[Dict[str, Any]]], compat: Dict[str, Any], op_status: Optional[Dict[str, Any]] = None) -> None:
    all_attack_ids = {aid for alert in observed_alerts for aid in extract_attack_ids(alert)}
    if op_status:
        if isinstance(op_status.get("links"), list):
            write_json(session_dir / "caldera_links.json", op_status.get("links"))
        if isinstance(op_status.get("potential_links"), list):
            write_json(session_dir / "caldera_potential_links.json", op_status.get("potential_links"))
        if isinstance(op_status.get("report"), dict):
            write_json(session_dir / "caldera_report.json", op_status.get("report"))
        if isinstance(op_status.get("event_logs"), list):
            write_json(session_dir / "caldera_event_logs.json", op_status.get("event_logs"))
    ...
    attack_summary = extract_operation_attack_summary(operation_info, compat, op_status, mapper)
    write_json(session_dir / "attack_summary.json", attack_summary)
    generate_attack_html_report(session_dir / "attack_report.html", cfg, session_id, operation_info, attack_summary)
    generate_d3fend_html_report(session_dir / "d3fend_report.html", session_id, operation_info, attack_summary, observed_attack_ids=all_attack_ids)
...
<style>body {{ font-family: Arial, sans-serif; margin: 24px; background: #111; color: #eee; }} table {{ border-collapse: collapse; width: 100%; background: #1c1c1c; }} th, td {{ border: 1px solid #333; padding: 10px; text-align: left; }} th {{ background: #222; }} a {{ color: #7db8ff; }} .box {{ background: #1c1c1c; border: 1px solid #333; padding: 12px; border-radius: 8px; margin-bottom: 16px; }}</style>
</head><body><h1>Per-source-IP reports</h1><p><strong>Session:</strong> {html.escape(session_id)}</p>
<div class='box'><strong>Global reports:</strong> <a href='../report.html'>Main report</a> | <a href='../attack_report.html'>MITRE ATT&amp;CK attack report</a> | <a href='../d3fend_report.html'>D3FEND defensive mapping report</a></div>
...
def launch_monitor_window_if_requested(config_path: str, session_dir: Path, duration_seconds: int) -> bool:
    ...
    cmd = ["powershell", "-NoExit", "-Command", f"python '{script_path}' --config '{Path(config_path).resolve()}' --monitor-only --session-dir '{session_dir.resolve()}' --monitor-seconds {duration_seconds}"]
...
def monitor_loop(cfg: LabConfig, client: Optional[CalderaClient], session_dir: Path, monitor_seconds: int, operation_info: Dict[str, Any], compat: Dict[str, Any], target_markers: Optional[Set[str]] = None, selector_info: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Dict[str, List[Dict[str, Any]]], Dict[str, Any]]:
    ...
    wazuh_debug = DebugLogger(session_dir / "debug_wazuh.ndjson", cfg.debug_wazuh)
    monitor = WazuhTailMonitor(cfg, line_queue, stop_event, debug_logger=wazuh_debug)
    monitor.start()
    ...
    if client and operation_info.get("id") is not None:
        poller = OperationPoller(client, resolve_op_id(operation_info["id"]), cfg.status_refresh_seconds, op_status, stop_event, debug_logger=client.debug)
        poller.start()
            ...
            if alert_matches_target(parsed, cfg, target_markers):
                ...
                if len(unmatched_samples) < 20:
                    unmatched_samples.append({
                        ...
                        "src": source_ip,
                        "preview": truncate(parsed.get("rule", {}).get("description", parsed.get("raw", "raw-line")), 180),
                    })
    ...
    if cfg.keep_wazuh_raw_lines and getattr(monitor, "raw_samples", None):
        write_json(session_dir / "debug_wazuh_raw_samples.json", list(monitor.raw_samples))
    wazuh_diag = {
        "total_lines_seen": total_lines_seen,
        "matched_lines_seen": matched_lines_seen,
        "unmatched_lines_seen": max(0, total_lines_seen - matched_lines_seen),
        "target_markers": sorted(target_markers or []),
        "unmatched_samples": unmatched_samples,
    }
    write_json(session_dir / "wazuh_diagnostics.json", wazuh_diag)
    op_status["diagnostics"] = build_operation_diagnostics(op_status, selector_info, compat)
    write_json(session_dir / "operation_diagnostics.json", op_status["diagnostics"])
...
def run_monitor_only(config_path: str, session_dir: str, monitor_seconds: int) -> int:
    ...
    build_reports(cfg, mapper, session_path, session_path.name, operation_info, observed_alerts, grouped, compat, op_status=op_status)
...
def list_caldera_for_agent(client: CalderaClient, cfg: LabConfig, interactive_select: bool) -> int:
    ...
    ranked = rank_adversaries_for_agent(adversaries, agent, abilities, cfg.noisy_only_mode, cfg.seed_only_mode)
    ...
    for item in ranked[:25]:
        adv = item["adversary"]
        compat = item["compatibility"]
        print(f"{truncate(adv.get('adversary_id') or adv.get('id'),24):<24} | {truncate(adv.get('name',''),28):<28} | compatible={compat['compatible_count']:<3} | missing={compat['missing_count']:<3} | seed_only={compat['seed_only_mode']}")
    return 0
...
def run(config_path: str, interactive_select: bool = False, list_only: bool = False) -> int:
    ...
    if cfg.auto_build_temp_adversary:
        temp_adv, selected_refs = build_temp_adversary_from_compatible(agent, selected_adv, abilities, cfg.max_temp_abilities, cfg.noisy_only_mode, cfg.seed_only_mode)
        if selected_refs:
            try:
                ...
                compat = compatibility_report(temp_adv, agent, abilities, noisy_only=cfg.noisy_only_mode, seed_only=cfg.seed_only_mode)
            except Exception as exc:
    ...
    if compat["compatible_count"] == 0:
        print("[-] Zero usable seed/noisy compatible abilities for this adversary/agent pair.", file=sys.stderr)
        return 5
    ...
    client.set_debug_dir(session_dir)
    client.debug.log("session_start", session_id=session_id, api_mode=client.api_mode)

    selector_payload = {"selected_agent": agent, "selected_adversary": selected_adv, "operation_adversary": operation_adv, "temp_adversary_info": temp_adversary_info}
    write_json(session_dir / "selector_info.json", selector_payload)
    write_json(session_dir / "compatibility_report.json", compat)
    write_json(session_dir / "temp_adversary_used.json", temp_adversary_info or {})
    write_json(session_dir / "selected_abilities_debug.json", {
        "selected_agent": {"paw": agent.get("paw"), "group": agent.get("group"), "platform": agent.get("platform")},
        "selected_adversary_id": selected_adv.get("adversary_id") or selected_adv.get("id"),
        "operation_adversary_id": operation_adv_id,
        "compatible_count": compat.get("compatible_count"),
        "referenced_ability_count": compat.get("referenced_ability_count"),
        "selected_abilities": (temp_adversary_info or {}).get("selected_abilities", []),
        "temp_adversary_saved": bool((temp_adversary_info or {}).get("saved")),
        "temp_adversary_error": (temp_adversary_info or {}).get("error"),
    })

    print(f"[+] CALDERA api_mode: {client.api_mode}")
    ...
    if temp_adversary_info and temp_adversary_info.get("saved"):
        print(f"[+] Temporary seed/noisy adversary created with {len(temp_adversary_info.get('selected_abilities', []))} abilities")

    operation_info = client.create_operation(operation_name, adversary_id=operation_adv_id, group=str(agent.get("group") or "red"))
    ...
    try:
        client.set_operation_state(resolve_op_id(op_id), "running", autonomous=True)
    ...
    operation_info["_api_mode"] = client.api_mode
    ...
    mapper = D3fendMapper(cfg.d3fend_mappings_url)
    selector_info = {"selected_agent": agent, "operation_adversary": operation_adv, "temp_adversary_info": temp_adversary_info}
    observed_alerts, grouped, op_status = monitor_loop(cfg, client, session_dir, total_monitor_seconds, operation_info, compat, target_markers=derive_target_markers(cfg, session_dir), selector_info=selector_info)
    operation_info["_polled_status"] = op_status
    ...
    write_json(session_dir / "caldera_report.json", op_status.get("report", {}))
    write_json(session_dir / "caldera_event_logs.json", op_status.get("event_logs", []))
    write_json(session_dir / "caldera_poll_history.json", op_status.get("poll_history", []))
    build_reports(cfg, mapper, session_dir, session_id, operation_info, observed_alerts, grouped, compat, op_status=op_status)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cyber lab orchestrator v6.4.4")
