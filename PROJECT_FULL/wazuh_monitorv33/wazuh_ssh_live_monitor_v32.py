...
def extract_src_ip(obj: Dict[str, Any], full_log: str) -> str:
    ...
    candidates = FULL_LOG_IP_RE.findall(full_log or "")
...
def matches_filters(row: EventRow, filters_cfg: Dict[str, Any]) -> bool:
    ...
    if contains:
        haystack = " ".join([
            row.timestamp, row.agent, row.agent_id, row.rule_id, row.level, row.groups,
            row.decoder, row.location, row.src_ip, row.mitre_ids, row.mitre_tactics,
            row.mitre_techniques, row.description, row.full_log,
        ]).lower()
...
def build_main_table(rows: List[EventRow]) -> Table:
    table = Table(title="Wazuh SSH Live Monitor v3.2", box=box.SIMPLE_HEAVY, expand=True, show_lines=False)
...
def build_layout(rows_main: List[EventRow], rows_suspicious: List[EventRow], stats: Dict[str, int], source_path: str, ssh_host: str) -> Group:
    main_table = build_main_table(rows_main)
...
class CSVExporter:
    ...
    def open(self):
        ...
        self.writer = csv.DictWriter(
            self.file,
            fieldnames=[
                "timestamp", "agent", "agent_id", "rule_id", "level", "groups",
                "decoder", "location", "src_ip", "mitre_ids", "mitre_tactics",
                "mitre_techniques", "description", "full_log", "suspicious",
                "suspicious_reason",
            ],
        )
...
class SSHLogStreamer:
    ...
    def connect(self):
        ...
        if key_file:
            ...
            pkey = None
            loaders = [paramiko.RSAKey.from_private_key_file, paramiko.Ed25519Key.from_private_key_file, paramiko.ECDSAKey.from_private_key_file]
            last_error = None
...
def main():
    ...
    stats = {"received": 0, "shown": 0, "filtered": 0, "suspicious": 0, "with_ip": 0, "with_mitre": 0, "json_errors": 0, "reconnects": 0}
