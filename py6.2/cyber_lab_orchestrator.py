...
def render_live_screen(session_name: str, remaining: int, total: int, observed_count: int, recent_rows: List[Dict[str, str]], op_status: Dict[str, Any], compat: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> None:
    ...
    print("CALDERA")
    print("-" * 120)
    print(f"op_id={op_id} | state={state} | group={host_group} | chain={len(chain) if isinstance(chain, list) else 'n/a'} | facts={len(facts) if isinstance(facts, list) else 'n/a'}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cyber lab orchestrator v6.2")
