...
Focus:
- Strict seed-only mode for Batch planner
- Only keeps Windows abilities that are:
  * executor-compatible
  * requirement-free
  * placeholder-free
  * non-empty commands
- Builds a minimal seed adversary (default 5 abilities)
- Verifies the temporary adversary is really used by the created operation
- Live monitor with:
  * time progress bar
...
class LabConfig:
    ...
    seed_only_mode: bool
    force_seed_adversary: bool
    strict_seed_limit: int
    ...
    def from_file(path: str) -> "LabConfig":
        ...
        return LabConfig(
            ...
            force_seed_adversary=bool(data.get("runtime", {}).get("force_seed_adversary", True)),
            strict_seed_limit=int(data.get("runtime", {}).get("strict_seed_limit", 5)),
        )
...
def unresolved_placeholders(ability: Dict[str, Any]) -> List[str]:
    return PLACEHOLDER_RE.findall(get_windows_executor_command(ability))
...
def noisy_score(ability: Dict[str, Any]) -> int:
    ...
    for kw in NOISY_WINDOWS_KEYWORDS:
        if kw in cmd or kw in name:
            score += 2
    if technique in SEED_PREFERRED_TECHNIQUES:
        score += 3
...
def build_strict_seed_adversary(agent: Dict[str, Any], source_adversary: Dict[str, Any], abilities: List[Dict[str, Any]], max_items: int, noisy_only: bool, seed_only: bool) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ability_map = {str(a.get("ability_id") or a.get("id")): a for a in abilities}
    ...
    temp_adv = {
        ...
        "name": f"Temp Strict Seed Windows - {source_adversary.get('name') or source_adversary.get('adversary_id')}",
        "description": f"Auto-generated strict seed adversary from {source_adversary.get('adversary_id') or source_adversary.get('id')} for agent {agent.get('paw')}",
...
def verify_operation_uses_expected_adversary(operation_info: Dict[str, Any], expected_adversary_id: str) -> Tuple[bool, str]:
    op_adv = operation_info.get("adversary") or {}
    op_adv_id = str(op_adv.get("adversary_id") or op_adv.get("id") or "")
    if not op_adv_id:
        return False, "operation payload missing adversary block"
    if str(expected_adversary_id) != op_adv_id:
        return False, f"operation adversary mismatch (expected {expected_adversary_id}, got {op_adv_id})"
    ordering = op_adv.get("atomic_ordering") or []
    if not isinstance(ordering, list) or len(ordering) == 0:
        return False, "operation adversary has empty atomic_ordering"
    return True, "ok"


def seed_adversary_applied(operation_info: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> bool:
    if not selector_info:
        return False
    temp_info = selector_info.get("temp_adversary_info") or {}
    temp_adv = temp_info.get("adversary") or {}
    temp_id = str(temp_adv.get("adversary_id") or "")
    if not temp_id:
        return False
    ok, _ = verify_operation_uses_expected_adversary(operation_info, temp_id)
    return ok
...
def relevant_ability_ids(operation_info: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> Set[str]:
    ...
    if selector_info:
        for key in ("operation_adversary",):
            op_adv = selector_info.get(key) or {}
            ordering = op_adv.get("atomic_ordering") or []
            if isinstance(ordering, list):
                for item in ordering:
                    if isinstance(item, str):
...
def decision_metrics(op: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> Dict[str, int]:
    ...
    metrics = {"planned_total": len(relevant_ids), "created": 0, "finished": 0, "success": 0, "error": 0}
...
def render_live_screen(session_name: str, remaining: int, total: int, observed_count: int, recent_rows: List[Dict[str, str]], op_status: Dict[str, Any], compat: Dict[str, Any], selector_info: Optional[Dict[str, Any]] = None) -> None:
    ...
    applied = seed_adversary_applied(op, selector_info)
    print("CALDERA")
    print("-" * 120)
    print(f"op_id={op_id} | state={state} | group={host_group} | chain={len(chain) if isinstance(chain, list) else 'n/a'} | facts={len(facts) if isinstance(facts, list) else 'n/a'} | seed_adversary_applied={applied}")
...
def run(config_path: str, interactive_select: bool = False, list_only: bool = False) -> int:
    ...
    if cfg.auto_build_temp_adversary:
        temp_adv, selected_refs = build_strict_seed_adversary(agent, selected_adv, abilities, cfg.strict_seed_limit, cfg.noisy_only_mode, cfg.seed_only_mode)
        if selected_refs:
            try:
    ...
    if cfg.force_seed_adversary and (not temp_adversary_info or not temp_adversary_info.get("saved")):
        print("[-] Forced seed adversary mode is enabled, but temporary strict seed adversary was not created.", file=sys.stderr)
        return 7

    compat = compatibility_report(operation_adv, agent, abilities, noisy_only=cfg.noisy_only_mode, seed_only=cfg.seed_only_mode)
    if compat["compatible_count"] == 0:
        print("[-] Zero usable strict seed abilities for this adversary/agent pair.", file=sys.stderr)
        return 5
    ...
    print(f"[+] Strict seed abilities selected: {compat['compatible_count']} / {compat['referenced_ability_count']}")
    if temp_adversary_info and temp_adversary_info.get("saved"):
        print(f"[+] Temporary strict seed adversary created with {len(temp_adversary_info.get('selected_abilities', []))} abilities")
    ...
    verification_ok, verification_msg = verify_operation_uses_expected_adversary(operation_info, operation_adv_id)
    operation_info["_seed_verification"] = {
        "expected_adversary_id": operation_adv_id,
        "ok": verification_ok,
        "message": verification_msg,
        "force_seed_adversary": cfg.force_seed_adversary,
    }
    ...
    if cfg.force_seed_adversary and not verification_ok:
        print(f"[-] Seed adversary verification failed: {verification_msg}", file=sys.stderr)
        return 8
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cyber lab orchestrator v6.4")
