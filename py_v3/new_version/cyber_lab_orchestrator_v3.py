...
class CalderaClient:
    ...
    @staticmethod
    def extract_operation_id(operation_info: Dict[str, Any]) -> Optional[str]:
        for key in ("id", "op_id", "operation_id"):
            value = operation_info.get(key)
            if value:
                return str(value)
        if isinstance(operation_info.get("data"), dict):
            for key in ("id", "op_id", "operation_id"):
                value = operation_info["data"].get(key)
                if value:
                    return str(value)
        return None
    ...
    def operation_state(operation: Optional[Dict[str, Any]]) -> str:
        ...
        if operation.get("finished") is True:
            return "finished"
        return "running"
...
def run(config_path: str) -> int:
    ...
    for idx, adv in enumerate(adversaries, start=1):
        ...
        try:
            op_info = caldera.create_operation(op_name, adv_id)
        except Exception as exc:
            op_info = {
                "name": op_name,
                "adversary_id": adv_id,
