...
What this version adds compared to v2:
...
class CalderaClient:
    ...
    @classmethod
    def extract_operation_id(cls, operation_info: Any) -> Optional[str]:
        info = cls._coerce_mapping(operation_info)
        for key in ("id", "op_id", "operation_id", "name"):
            value = info.get(key)
            if value:
                return str(value)
        data = info.get("data")
        ...
        if isinstance(items, list):
            for item in items:
                if isinstance(item, dict):
                    nested = cls.extract_operation_id(item)
                    if nested:
                        return nested
        return None

    @classmethod
    def operation_state(cls, operation: Optional[Any]) -> str:
        if not operation:
            return "unknown"
        info = cls._coerce_mapping(operation)
...
def print_wazuh_table_row(index: int, ts: str, level: int, src_ip: str, agent: str, description: str) -> None:
    ...
    row = f"{ts:<21} | {str(level):<5} | {src_ip:<15} | {agent:<15} | {description:<90}"
...
def run(config_path: str) -> int:
    ...
    for idx, adv in enumerate(adversaries, start=1):
        ...
        try:
            op_info = caldera._coerce_mapping(caldera.create_operation(op_name, adv_id))
        except Exception as exc:
            op_info = {
                "name": op_name,
                "adversary_id": adv_id,
        ...
        op_id = caldera.extract_operation_id(op_info) or f"unknown-{idx}"
        shared.update(
            current_operation_name=op_name,
            current_adversary_id=adv_id,
